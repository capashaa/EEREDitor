    ...
    class ToolPicker : Tool
    {
            ...
            : base(editArea)
        {
            ...
            if (settings.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    Color color = ColorTranslator.FromHtml(settings.hex);
                    Color2ID(color,settings.colorExact);
                }
                else
                {
                    ...
                    {
                        Color2ID(cd.Color, settings.colorExact);
                    }
                }
            }
        }

        private void Color2ID(Color color,bool exactly)
        {
            ...
            var bid = closestColor(color,exactly);
            editArea.MainForm.setBrick(bid,true);
        }

        private int closestColor(Color col,bool exactly)
        {
            int black = 0;
            double num = 999.0;
            double d = 0.0;
            foreach (KeyValuePair<Color,int> id in colblocks)
            {
                ...
                else
                {
                    Color color2 = id.Key;
                    d = 0.0;
                    if (color2.R >= col.R) d += Math.Pow((double)(color2.R - col.R), 2.0);
                    else d += Math.Pow((double)(col.R - color2.R), 2.0);
                    if (color2.G >= col.G) d += Math.Pow((double)(color2.G - col.G), 2.0);
                    else d += Math.Pow((double)(col.G - color2.G), 2.0);
                    if (color2.B >= col.B) d += Math.Pow((double)(color2.B - col.B), 2.0);
                    else d += Math.Pow((double)(col.B - color2.B), 2.0);
                    d = Math.Sqrt(d);
                    if (d < num)
                    {
                        num = d;
